...
export default function fileUpload(userId, encodedFile) {
  const key = 'images/' + process.env.NODE_ENV + '/' + new Date().toISOString().substr(0, 10) +
    '/' + userId + '/' + String(Date.now()) + '.png';
  console.log(key);
  const imageType = encodedFile.split('/')[1].split(';')[0];
  const imageFile = encodedFile.split(';')[1].split(',')[1];
  let resp = {};
  if (!imageType.match(/(gif|jpe?g|png)/i)) {
    resp = {
      status: 500,
  ...
  // signedUrl = url;
  const bitmap = new Buffer(imageFile, 'base64');
